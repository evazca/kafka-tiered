...
class LocalLogTest {
  ...
  def testLogDeleteSegmentsSuccess(): Unit = {
    ...
    val deletedSegments = List[LogSegment]() ++ log.deleteAllSegments().asScala
  ...
  private def testDeleteSegmentFiles(asyncDelete: Boolean): Unit = {
    ...
    val toDelete = List[LogSegment]() ++ log.segments.values
    LocalLog.deleteSegmentFiles(toDelete.asJava, asyncDelete, log.dir, log.topicPartition, log.config, log.scheduler, log.logDirFailureChannel, "")
    if (asyncDelete) {
      toDelete.foreach {
        segment =>
  ...
  def testDeletableSegmentsFilter(): Unit = {
...

    {
      ...
      assertEquals(expected, deletable.asScala.toList)
    }
  ...
  def testDeletableSegmentsIteration(): Unit = {
    ...
    assertEquals(10L, log.segments.numberOfSegments)
    assertEquals(log.segments.nonActiveLogSegmentsFrom(0L).toSeq, deletableSegments.asScala.toSeq)
  }
  ...
  def testTruncateFullyAndStartAt(): Unit = {
    ...
    val deleted = List[LogSegment]() ++ log.truncateFullyAndStartAt(10L).asScala
  ...
  def testTruncateTo(): Unit = {
    ...
    val deleted = List[LogSegment]() ++ log.truncateTo(7L).asScala
    assertEquals(expected, deleted)
