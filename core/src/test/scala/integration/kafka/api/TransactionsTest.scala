...
class TransactionsTest extends IntegrationTestHarness {
  ...
  def testBumpTransactionalEpoch(quorum: String): Unit = {
    ...
    try {
      ...
      producerStateEntry =
        brokers(partitionLeader).logManager.getLog(new TopicPartition(testTopic, 0)).get.producerStateManager.activeProducers.asScala(producerId)
  ...
  def testFailureToFenceEpoch(quorum: String): Unit = {
    ...
    producerStateEntry =
      brokers(partitionLeader).logManager.getLog(new TopicPartition(topic1, 0)).get.producerStateManager.activeProducers.asScala(producerId)
