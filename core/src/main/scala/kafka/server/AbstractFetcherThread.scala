...
abstract class AbstractFetcherThread(name: String,
  ...
  /**
   * Returns the leader's local-log-start-offset and the respective leader-epoch tuple from leader replica for the given
   * topic partition and the current leader-epoch of leader in this replica.
   *
   * @param topicPartition topic partition
   * @param currentLeaderEpoch current leader epoch of leader on this replica.
   */
  protected def fetchEarliestLocalOffsetFromLeader(topicPartition: TopicPartition, currentLeaderEpoch: Int): (Int, Long)

  /**
   * Returns the leader's log-start-offset and the respective leader-epoch tuple from leader replica for the given topic
   * partition and the current leader-epoch of leader in this replica.
   *
   * @param topicPartition topic partition
   * @param currentLeaderEpoch current leader epoch of leader on this replica.
   */
