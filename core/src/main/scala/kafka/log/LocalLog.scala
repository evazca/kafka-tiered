...
import kafka.server.{FetchDataInfo, LogDirFailureChannel}
...
import org.apache.kafka.server.log.internals.{AbortedTxn, LogOffsetMetadata, OffsetPosition}
...
class LocalLog(@volatile private var _dir: File,
  ...
  private[log] def collectAbortedTransactions(logStartOffset: Long, baseOffset: Long, upperBoundOffset: Long): List[AbortedTxn] = {
    ...
    val allAbortedTxns = new ListBuffer[AbortedTxn]
